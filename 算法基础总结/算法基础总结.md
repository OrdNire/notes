...
![二分0](D:\个人资料\Notes\算法基础总结\img\二分0.png)
...
![前缀和](D:\个人资料\Notes\算法基础总结\img\前缀和.png)
...
![dijkstra问题图1](D:\个人资料\Notes\算法基础总结\img\dijkstra问题图1.png)
...
![bellman-ford问题图1](D:\个人资料\Notes\算法基础总结\img\bellman-ford问题图1.png)
...
![二分图01](D:\个人资料\Notes\算法基础总结\img\二分图01.png)
...
> 背包问题是一类特别经典的背包问题，很多动态规划的问题其实都可以类似于背包问题

背包问题解决的主要是，有 $N$ 件物品和一个容量为 $V$ 的背包，每件物品有其价值和体积，问如何选择可以使得背包中物品的价值最大

根据可选物品的种类和数量，背包问题又分为了多种类型的经典问题
...
状态表示：$F_{i,j}$ 表示从 1 ~ i 中选，体积不超过 j 的最大价值

状态转移：$F_{i,j} = max(F_{i - 1,j},F_{i - 1,j - v_i} + w_i)$

状态转移分析：

假设我们已经分析到第 $i$ 个物品，那么由$F_{i,j}$ 的状态表示，我们可以有两种状态，选第 $i$ 件物品和不选第 $i$ 件物品

如果选第 $i$ 件物品，那么状态就是从 1 ~ (i - 1) 中选的物品，体积不超过 $j - v_i$ 的状态中，加上了这件物品的价值

如果不选第 $i$ 件物品，那么状态就等价于从 1 ~ (i - 1) 中选物品，体积不超过 j 的状态
...
状态转移：$F_{i,j} = max(F_{i - 1,j},F_{i - 1,j - v_i} + w_i,F_{i - 1,j - 2*v_i} + 2*w_i....+F_{i - 1,j - k*v_i} + k*w_i) (k*v_i <= j)$
...
1. $F_{i,j} = max(F_{i - 1,j},F_{i - 1,j - v_i} + w_i,F_{i - 1,j - 2*v_i} + 2*w_i....+F_{i - 1,j - k*v_i} + k*w_i)$
2. $F_{i,j - v} = max($        $F_{i - 1,j - v_i},F_{i - 1,j - 2*v_i} + w_i,F_{i - 1,j - 3*v_i} + 2*w_i....)$

所以可以发现，式1后面的每一项都比式2后面的每一项多 w

所以状态转移方程可以优化为 $F_{i,j} = max(F_{i - 1,j},F_{i,j - v_i} + w_i)$
...
> 多重背包问题就是限制了每件物品选择的个数

这个其实就是在完全背包问题上加上了选的个数限制
...
> 最长上升子序列问题是线性DP中一类经典的问题

状态表示：$F_i$ 表示以 $i$ 结尾的严格单调递增的序列长度的最大值

状态转移：$F_i = max(F_i,F_j + 1) (a_j < a_i)$ $j$ 从 $1$ 到 $i - 1$

状态转移分析：

由状态表示可知，我们可以枚举前面所有的序列元素，如果是单调递增的，就从那个元素转移过来，就要加 $1$ 表示元素长度增加了一个，求所有的最大值即可
